        ...
        with torch.no_grad():
            for x, y in dataloader:
                ...
                try:
                    ...
                    sumerrorsquared=0
                    stddevsum=0
                    for inloc, outloc in zip(input, output):
                        for innum, outnum in zip(inloc, outloc):
                            if innum.item()>0:
                                sumerror += torch.abs(outnum-innum)
                                sumerrorsquared += ((outnum-innum) ** 2)
                                stddevsum += ((outnum - avg) ** 2)

                    batch_mse = sumerrorsquared/batch_size
                    batch_mae = sumerror/batch_size
                    batch_stddev = stddevsum/batch_size
                    batch_mase = batch_mae/np.sqrt(batch_stddev)
                    meanSquaredError += batch_mse * batch_size
                    meanAverageError += batch_mae * batch_size
                    meanStddev += batch_stddev * batch_size
                    meanMase += batch_mase * batch_size
                    processed_batches += 1
